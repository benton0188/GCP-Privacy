...
async function createFinding(resource) {
  const [newFinding] = await client.createFinding({
    ...
    findingId: 'BasicRoleAdd' + Date.now(),
    finding: {
      state: 'ACTIVE',
      severity: 'MEDIUM',
...
exports.createSccFinding = (event, context) => {
  ...
  //? Buffer.from(event.data, 'base64').toString()                                                    ## Change code to support use case. E.G. Check label of workload and if Confidential or GDPR related, check the protection level in cloudkms.googleapis.com/CryptoKeyVersion and create SCC alert. 
  //: 'Hello, World';
